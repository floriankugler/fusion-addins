# WorkingModel.objectVisibility Property

Parent Object: [WorkingModel](WorkingModel.md)  

## Description

Returns the ObjectVisibility object associated with this design which controls which objects are displayed in the graphics window. This is the equivalent of the "Object Visibility" settings in the Display Settings drop-down in the navigation toolbar at the bottom of the Fusion graphics window.

## Syntax

"workingModel_var" is a variable referencing a WorkingModel object.  

```python
# Get the value of the property.
propertyValue = workingModel_var.objectVisibility
```

## Property Value

This is a read only property whose value is an [ObjectVisibility](ObjectVisibility.md).

## Version

Introduced in version November 2025  

