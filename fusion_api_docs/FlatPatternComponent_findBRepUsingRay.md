# FlatPatternComponent.findBRepUsingRay Method
...
returnValue = flatPatternComponent_var.findBRepUsingRay(originPoint, rayDirection, entityType)

# Uses optional arguments.
returnValue = flatPatternComponent_var.findBRepUsingRay(originPoint, rayDirection, entityType, proximityTolerance, visibleEntitiesOnly, hitPoints)
```
...
| --- | --- | --- |
| originPoint | [Point3D](Point3D.md) | Input point that defines the origin of the ray. The search for entities begins at this point. |
| rayDirection | [Vector3D](Vector3D.md) | Input vector that defines the direction of the ray. The ray is infinite so the length of the vector is ignored. |
