...
```python
returnValue = graph_var.getOutputNode(outputNodeType)
```
...
| Type | Description |
|----|----|
| [GraphNode](GraphNode.md) | The output node if present in the graph, null otherwise. |

## Parameters

| Name | Type | Description |
|----|----|----|
| outputNodeType | [GraphOutputNodeTypes](GraphOutputNodeTypes.md) | The type of the output graph node. Not every graph has every type; the primary graph has all four ouput node types, the cell graph only has OutputDensity. |
