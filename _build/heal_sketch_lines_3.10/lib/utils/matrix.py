...
def transform_from_root(origin: Point3D, axis1: Vector3D, axis2: Vector3D) -> Matrix3D:
    t = Matrix3D.create()
    if axis1.length == 0 or axis2.length == 0:
        raise ValueError("axis1 and axis2 must be non-zero")
    if axis1.isParallelTo(axis2):
        raise ValueError("axis1 and axis2 must not be parallel")
