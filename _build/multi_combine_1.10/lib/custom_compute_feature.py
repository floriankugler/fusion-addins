...
class CustomComputeFeature(Addin):
    ...
    def __init__(self, runtime_info: RuntimeInfo):
        try:
            ...
            self._custom_shutdown = False

            existing_edit_cmd_def = self.ui.commandDefinitions.itemById(self.edit_command_id)
            if existing_edit_cmd_def:
                existing_edit_cmd_def.deleteMe()
    ...
    def shutdown(self):
        if self._custom_shutdown:
            return
        custom_shutdown_ok = True
        try:
            cmd_def = self.ui.commandDefinitions.itemById(self.edit_command_id)
            if cmd_def:
                cmd_def.deleteMe()
        except:
            custom_shutdown_ok = False
            utils.fusion.handleException()
        finally:
            super().shutdown()
        if custom_shutdown_ok and self._shutdown:
    ...
    def _execute(self, args: adsk.core.CommandEventArgs):
        if self.inputs is None:
            raise RuntimeError("Custom feature inputs are not initialized.")
        ...
        with self.compute_disabled():
            ...
            try:
                ...
                self.log_exception_traceback("create_execute", e)
    ...
    def _execute_preview(self, args: adsk.core.CommandEventArgs):
        ...
        with self.compute_disabled():
            try:
                ...
                args.isValidResult = False
                self.log_exception_traceback("execute_preview", e)
                if self.runtime_info.dev_mode: raise e
            else:
                self.showError(None)
    ...
    def _edit_execute(self, args: adsk.core.CommandEventArgs):
        if self.inputs is None:
            raise RuntimeError("Custom feature inputs are not initialized.")
        if self._edited_custom_feature is None:
            raise RuntimeError("Edited custom feature is not initialized.")
        self.update_inputs_from_ui()
        features_inside_component: list[adsk.fusion.Feature] = []
        features_outside_component: list[adsk.fusion.Feature] = []
        ...
        with self.compute_disabled():
            ...
            try:
                ...
                args.executeFailed = True
                args.executeFailedMessage = "An error occurred during execution."
                self.log_exception_traceback("edit_execute", e)
                if self.runtime_info.dev_mode: raise e
            else:
                self._edited_custom_feature.timelineObject.rollTo(True)
    ...
    def _compute(self, args: adsk.fusion.CustomFeatureEventArgs):
        ...
        try:
            ...
            self.log_exception_traceback("custom_compute", e)
            if self.runtime_info.dev_mode: raise e
        finally:
            self.inputs = None
