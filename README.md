...
- **Face Cutout**
  ...
  Creates one inset cutout from a selected planar face using only native Fusion timeline features: a linked offset sketch, a new-body extrude, an optional fillet, and a cut combine. Existing inner face loops are preserved using a separate inner-feature inset and can be connected to the outer perimeter with material tabs.
- **Concealed Hinge**
