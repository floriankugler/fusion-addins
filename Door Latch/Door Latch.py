...
class DoorLadgeInputs(Inputs.Inputs):
    ...
    def __init__(self, units_manager: adsk.core.UnitsManager):
        ...
        self.type = Inputs.DropDownInput('type', 'Latch type', self.types.items(), 0, 'The type of latch to use.')
...
class DoorLatch(CustomComputeFeature.CustomComputeFeature):
    ...
    def carcass_holes(self, carcass_edge: adsk.fusion.BRepEdge, positions: list[Vector3D]) -> adsk.fusion.BRepBody:
        ...
        cyl = utils.brep.transformed(cyl, utils.matrix.translation_matrix(Vector3D.create(0, 2.5 - gap, 0)))
        group = utils.brep.union([
            utils.brep.transformed(cyl, utils.matrix.translation_matrix(Vector3D.create(-1.4, 0, 0))),
            utils.brep.transformed(cyl, utils.matrix.translation_matrix(Vector3D.create(1.4, 0, 0)))
        ])
    ...
    def door_holes(self, carcass_edge: adsk.fusion.BRepEdge, door_edge: adsk.fusion.BRepEdge, door_face: adsk.fusion.BRepFace, positions: list[Vector3D]) -> adsk.fusion.BRepBody:
        ...
        finger_hole = utils.brep.transformed(utils.brep.cylinder(1.25, -thickness), utils.matrix.translation_matrix(Vector3D.create(0, 3 + distance, 0)))

        union = utils.brep.union([screw_holes, finger_hole])

        return utils.brep.place_body_on_face_at_positions(union, door_face, door_edge, positions)
